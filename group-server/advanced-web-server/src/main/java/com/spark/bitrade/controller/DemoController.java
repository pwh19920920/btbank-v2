...
public class DemoController extends ApiController {
    ...
    @ApiOperation(value = "测试Token", notes = "用于自动化测试")
    @ApiImplicitParam(value = "会员ID", defaultValue = "0", name = "memberId", dataTypeClass = Long.class, required = true)
    @GetMapping("/demo/token")
    public MessageRespResult<String> testToken(@RequestParam("memberId") Long memberId) {
        HttpJwtToken instance = HttpJwtToken.getInstance();
        Date date = new Date();
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        calendar.add(Calendar.DATE, 30);
        MemberClaim memberClaim = MemberClaim.builder().subject("test").audience("0").username("test")
                .userId(memberId).issuedAt(date).expiresAt(calendar.getTime()).build();
        String tokenWithClaim = instance.createTokenWithClaim(memberClaim);
        return MessageRespResult.success4Data(tokenWithClaim);
    }
}
