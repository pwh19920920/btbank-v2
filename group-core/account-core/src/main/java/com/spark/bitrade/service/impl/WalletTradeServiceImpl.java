...
//    @LcnTransaction(propagation = DTXPropagation.SUPPORTS)
    ...
    public Boolean trade(WalletTradeEntity tradeEntity) throws MessageCodeException {
        ...
        writeWalletChangeAndTransactionRecord(tradeEntity, memberWallet);

        log.info("账户资金操作--结束:{}", tradeEntity);
        return true;
    }
...
//    @LcnTransaction(propagation = DTXPropagation.SUPPORTS)
    @Transactional(rollbackFor = Exception.class)
    public Boolean trade2(WalletTradeEntity tradeEntity, boolean btBank) throws MessageCodeException {
        log.info("账户资金操作--开始:{}", tradeEntity);
        MemberWallet memberWallet = memberWalletService.findByCoinAndMemberId(tradeEntity.getCoinId(), tradeEntity.getMemberId());
        AssertUtil.notNull(memberWallet, AcctMsgCode.MISSING_ACCOUNT);
        ...
        //账户操作，失败后则直接抛异常
        if (btBank) {
            if (!memberWalletService.tradeAllowNegative(memberWallet.getId(), tradeEntity.getTradeBalance(),
                    tradeEntity.getTradeFrozenBalance(), tradeEntity.getTradeLockBalance())) {
                log.error("try操作将增加的资产放到冻结资产失败！钱包ID={}", memberWallet.getId());
                ExceptionUitl.throwsMessageCodeException(AcctMsgCode.ACCOUNT_BALANCE_TRADE_FAILED);
            }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Boolean trade(WalletTradeEntity tradeEntity, BigDecimal minimum) throws MessageCodeException {
        log.info("账户资金操作--开始:{}", tradeEntity);
        MemberWallet memberWallet = memberWalletService.findByCoinAndMemberId(tradeEntity.getCoinId(), tradeEntity.getMemberId());
        AssertUtil.notNull(memberWallet, AcctMsgCode.MISSING_ACCOUNT);
        ...
        // 余额必须大于等于最小额
        if (memberWallet.getBalance().compareTo(minimum) < 0) {
        ...
        //账户操作，失败后则直接抛异常
        if (!memberWalletService.trade(memberWallet.getId(), tradeEntity.getTradeBalance(),
                tradeEntity.getTradeFrozenBalance(), tradeEntity.getTradeLockBalance(), minimum)) {
            log.error("try操作将增加的资产放到冻结资产失败！钱包ID={}", memberWallet.getId());
            ExceptionUitl.throwsMessageCodeException(AcctMsgCode.ACCOUNT_BALANCE_TRADE_FAILED);
        }
    ...
    private void writeWalletChangeAndTransactionRecord(WalletTradeEntity tradeEntity, MemberWallet memberWallet) {
        //写变更日志，记录写入“系统级别的资金变动记录”
