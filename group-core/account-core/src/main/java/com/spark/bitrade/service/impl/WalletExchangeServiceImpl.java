...
//    @LcnTransaction
    ...
    public boolean exchange(WalletExchangeEntity exchangeEntity, boolean btBank) throws MessageCodeException {
        ...
        if (BigDecimalUtil.lt0(sourceEntity.getTradeBalance())) {
            if (!walletTradeService.trade2(sourceEntity, btBank)) {
                ExceptionUitl.throwsMessageCodeException(AcctMsgCode.ACCOUNT_BALANCE_TRADE_FAILED);
            }
            if (!walletTradeService.trade2(targetEntity, btBank)) {
                ExceptionUitl.throwsMessageCodeException(AcctMsgCode.ACCOUNT_BALANCE_TRADE_FAILED);
            }
        } else {
            if (!walletTradeService.trade2(targetEntity, btBank)) {
