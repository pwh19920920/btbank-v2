...
public class InternalExchangeFastOrderController {
    ...
    @ApiOperation(value = "获取最新币种行情", notes = "获取最新币种行情")
    @ApiImplicitParams({
            @ApiImplicitParam(value = "币种", name = "unit", required = true,type = "path"),
    })
    @RequestMapping(value = "/thumb/{unit}",
            method = {RequestMethod.GET, RequestMethod.POST})
    public MessageRespResult<String> thumb(@PathVariable("unit") String unit) {

        return MessageRespResult.success4Data(JSON.toJSONString(AutoRefreshTask.thumbs.get(StringUtils.upperCase(unit))));
    }
}
