...
public class MemberWalletController extends ApiController {
     ...
     */
    @PostMapping("/trade2")
    public MessageRespResult<Boolean> trade2(@RequestBody WalletTradeEntity tradeEntity, @RequestParam("minimum") BigDecimal minimum) {
        if (tradeEntity == null || tradeEntity.getCoinUnit() == null || minimum == null) {
            throw new MessageCodeException(CommonMsgCode.INVALID_PARAMETER);
        }
        if (StringUtils.isEmpty(tradeEntity.getCoinId())) {
            tradeEntity.setCoinId(coinService.findByUnit(tradeEntity.getCoinUnit()).getName());
        }
        return MessageRespResult.success4Data(this.walletTradeService.trade(tradeEntity, minimum));
    }
    ...
    public MessageRespResult<Boolean> exchange(@RequestBody WalletExchangeEntity exchangeEntity) {
        return doExchange(exchangeEntity, false);
    }
    ...
    public MessageRespResult<Boolean> exchange2(@RequestBody WalletExchangeEntity exchangeEntity) {
        return doExchange(exchangeEntity, true);
    }

    private MessageRespResult<Boolean> doExchange(WalletExchangeEntity exchangeEntity, boolean btBank) {
        ...
        return MessageRespResult.success4Data(this.walletExchangeService.exchange(exchangeEntity, btBank));
    }
