...
import com.spark.bitrade.constant.MinerOrderTransactionType;
...
//        if (count == null) count = 0L;
    ...
    public MinerOrdersVO getMinerOrders(int page, int size,Integer type ){
        IPage<BtBankMinerOrder> minerOrderIPage = baseMapper.queryNewOrdersListByrange(new Page<>(page, size), type);
        MinerOrdersVO minerOrdersVO = new MinerOrdersVO();
        minerOrdersVO.setContent(minerOrderIPage.getRecords());
        minerOrdersVO.setTotalElements(minerOrderIPage.getTotal());
        return minerOrdersVO;
